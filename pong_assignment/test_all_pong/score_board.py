...
class ScoreBoard:
    def __init__(self, x, y, width, height):
        ...
        self.mLeftText.setColor((150, 20, 20))
        self.mRightScore = 0
        self.mRightText = Text(str(self.mRightScore), self.mX + 60, self.mY + 22)
        self.mRightText.setColor((20, 20, 150))
        self.mServeStatus = 1
        self.mServeIndicator = Indicator(self.mX + 3, self.mY + 20, 5, 5, (200, 200, 200))
    ...
    def swapServe(self):
        if not self.isGameOver():
            if self.mServeStatus == 1:
                self.mServeStatus = 2
                self.mServeIndicator.setX(self.mX + 70)
            else:
                self.mServeStatus = 1
                self.mServeIndicator.setX(self.mX + 3)
